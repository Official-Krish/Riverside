...
export function useTrackOperations(
  ...
  const handleDeleteClip = useCallback((trackIndex: number, clipId: string) => {
    setTracks((prev) =>
      prev.map((track, i) => {
        if (i !== trackIndex) return track;
        return { ...track, clips: track.clips.filter((c) => (c.id ?? c.sourceAssetId) !== clipId) };
      })
    );
  }, [setTracks]);
    ...
    setTracks((prevTracks) =>
      prevTracks.map((track, i) => {
        ...
        for (const clip of track.clips) {
          ...
          if (position === "start" && Math.abs(timelineMs - clipStart) < 500) {
            updated.push({
              ...clip,
              transitionStart: {
        ...
        for (let c = 0; c < clips.length; c++) {
          ...
          if (timelineMs > clipStart && timelineMs < clipEnd) {
            ...
            if (distToStart < EDGE_THRESHOLD) {
              updated.push({
                ...clip,
                transitionStart: { type: transitionType as any, durationMs: 500, easing: "ease-in-out" as const },
              });
              continue;
            }
            if (distToEnd < EDGE_THRESHOLD) {
              updated.push({
                ...clip,
                transitionEnd: { type: transitionType as any, durationMs: 500, easing: "ease-in-out" as const },
              });
              continue;
            }
