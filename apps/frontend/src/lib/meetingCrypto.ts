...
export function persistMeetingCek(meetingId: string, cek: Uint8Array) {
    window.sessionStorage.setItem(
        `${MEETING_CEK_STORAGE_PREFIX}${meetingId}`,
        JSON.stringify(Array.from(cek))
    );
}

export function readMeetingCek(meetingId: string) {
    const stored = window.sessionStorage.getItem(`${MEETING_CEK_STORAGE_PREFIX}${meetingId}`);
    if (!stored) {
        return null;
    }
...
async function deriveChunkIv(participantId: string, sequenceNumber: number) {
    const payload = `${participantId}:${sequenceNumber}`;
...
export async function encryptMeetingChunk({
    ...
    const cek = readMeetingCek(meetingId);
    if (!cek) {
        throw new Error("Meeting CEK not found. Rejoin the meeting to continue recording.");
    }
    ...
    const iv = await deriveChunkIv(participantId, sequenceNumber);
