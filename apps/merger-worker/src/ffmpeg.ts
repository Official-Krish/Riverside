...
export const ffmpegBin: string = getEnv(
  "FFMPEG_PATH",
  ffmpegStatic || "ffmpeg",
);
export const ffprobeBin: string = getEnv(
  "FFPROBE_PATH",
  ...
  await new Promise<void>((resolve, reject) => {
    ...
    const doResolve = async (error?: Error) => {
      ...
      if (error) {
        ...
        log(
          `[${label}] Completed successfully in ${duration}ms (${progressLines} progress lines)`,
        );
        resolve();
      }
    };
    ...
    const resetTimeout = () => {
      ...
      timeoutHandle = setTimeout(() => {
        ...
        log(
          `[${label}] No FFmpeg progress for ${idleFor}ms, killing process...`,
        );
        if (progressLines === 0) {
          log(
            `[${label}] WARNING: No FFmpeg output received - process may be stuck`,
          );
        }
        doResolve(
          new Error(`FFmpeg stalled after ${idleFor}ms without progress`),
        );
      }, timeoutMs);
    };
    ...
    ffmpeg.stderr.on("data", (data) => {
      ...
      stderr += output;
      // Cap stderr to prevent memory leak on long encodes
    ...
    ffmpeg.on("close", (code) => {
      ...
      doResolve(
        new Error(`FFmpeg failed with code ${code}: ${stderr.slice(-4096)}`),
      );
    });
    ...
    ffmpeg.on("error", (error) => {
      if ((error as NodeJS.ErrnoException).code === "ENOENT") {
        doResolve(
          new Error(`ffmpeg binary not found. Checked path: ${ffmpegPath}`),
        );
        return;
      }
      doResolve(error);
...
export async function getVideoDuration(
  ...
  return await new Promise<number>((resolve, reject) => {
    ...
    ffprobe.on("error", (error) => {
      ...
      if ((error as NodeJS.ErrnoException).code === "ENOENT") {
        reject(
          new Error(`ffprobe binary not found. Checked path: ${ffprobePath}`),
        );
        return;
      }
      reject(error);
