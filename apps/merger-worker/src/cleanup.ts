...
export async function cleanupTempDir(tempDir: string, log: (message: string) => void): Promise<void> {
...
export async function cleanupLegacyLocalChunks(recordingsRoot: string, meetingId: string): Promise<void> {
    const rawDir = path.join(recordingsRoot, meetingId, "raw");
    await fs.rm(rawDir, { recursive: true, force: true }).catch(() => undefined);
}

export async function cleanupLegacyRecordingsTmp(recordingsRoot: string, log: (message: string) => void): Promise<void> {
    ...
    try {
        ...
        log(`[cleanupLegacyRecordingsTmp] Note: Could not access tmp directory (may be normal)`);
    }
}